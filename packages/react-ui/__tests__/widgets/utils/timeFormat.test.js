...
import { formatTimeRange } from '../../../src/widgets/TimeSeriesWidgetUI/utils/timeFormat';

describe('timeFormat', () => {
  describe('formatTimeRange', () => {
    function defineCases(cases) {
      cases.forEach(({ date, expected, title, ...params }) =>
        it(`${date} / ${params.stepMultiplier ?? 1} ${params.stepSize} ===> ${expected} ${
          title ? `- ${title}` : ''
        }`, () =>
          expect(formatTimeRange({ date: new Date(date), ...params })).toBe(expected))
      );
    }
    describe('second', () => {
      defineCases([
