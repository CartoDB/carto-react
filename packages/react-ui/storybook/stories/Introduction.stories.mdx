...
We have support for `React` 17 & 18.
...
- `Deprecated`: Legacy component that might be currently in use, but shouldn`t be used in new projects
- `In development`: Work in progress or experimental. Not ready to use in production yet
- `Ready to review`: Ready to review and validation by CARTO design team and other stakeholders, prior to be consider fully done
- `Validated`: Fully validated and ready-to-use implementation
- `Needs update`: Needs an update due to some design changes, but can be used in this state
